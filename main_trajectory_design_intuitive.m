...
for n = 1 : 1 : Nro
    ...
    subplot(5,4,[1 2 5 6]);p1 = plot(10^3 * t_vec,k_x,'Color',blues(10,:),'LineWidth',3);hold on;
    p2 = plot(10^3 * t_vec,k_y,'Color',blues(5,:),'LineWidth',3);
    p01 = plot([0 10^3 * t_vec(1)],[0 0],'Color',blues(10,:),'LineWidth',3);
    p02 = plot([t_vec(end) * 10^3 13*10^3],[0 0],'Color',blues(10,:),'LineWidth',3);    
    p3 = scatter(10^3 * t_adc,0 * t_adc,50,colors);hold off;axis([0 3 * 10^3 * t_grad(end) -4 5]);
    title('Gradient waveform');xlabel('Time [ms]');ylabel('G_{str} [mT/m]');legend([p1 p2 p3],{'G_{x}','G_{y}','ADC'},'Orientation','horizontal','Location','north')
